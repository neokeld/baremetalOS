...
{
    int arg_value0, arg_value1;
    if (nb_args == 2)
    {
	arg_value0 = atoi(args[0]);
	if(arg_value0 != 2 && arg_value0 != 3) {
	    console_print("Bad argument : Not a gpio number.\r\n");
	    return -1;
	}
	arg_value1 = atoi(args[1]);
	if(arg_value1 > 0 && ((arg_value0 == 2 && arg_value1 < GPIO2_PIN_MAP_SIZE && gpio2_pin_map[arg_value1][0] != -1) ||(arg_value0 == 3 && arg_value1 < GPIO3_PIN_MAP_SIZE && gpio3_pin_map[arg_value1][0] != -1)) )
	    {
		if(gpio_is_activated(arg_value0, arg_value1))
		{
		    gpio_output_set(arg_value0, arg_value1);
		    gpio_deactivate(arg_value0, arg_value1);
		}
		else
		{
		    gpio_output_set(arg_value0, arg_value1);
		    gpio_activate(arg_value0, arg_value1);
		}
	    }
	    else
	    {
